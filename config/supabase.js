// config/supabase.js
// Secure Supabase Configuration - Emergency Fix for Render Deployment
const getSupabaseConfig = () => {
    // Check if environment configuration is available (generated by build.js)
    if (typeof window !== 'undefined' && window.ENV_CONFIG) {
        const config = {
            url: window.ENV_CONFIG.SUPABASE_URL,
            anonKey: window.ENV_CONFIG.SUPABASE_ANON_KEY
        };
        
        // Validate configuration
        if (!config.url || !config.anonKey) {
            console.error('❌ Supabase configuration incomplete in ENV_CONFIG:');
            console.error('- SUPABASE_URL:', config.url ? '✅ Set' : '❌ Missing');
            console.error('- SUPABASE_ANON_KEY:', config.anonKey ? '✅ Set' : '❌ Missing');
            throw new Error('Incomplete Supabase configuration. Please contact administrator.');
        }
        
        console.log('✅ Supabase configuration loaded from build-time environment');
        return config;
    }
    
    // EMERGENCY FALLBACK: Check if we're on the deployed domain
    if (typeof window !== 'undefined' && 
        (window.location.hostname.includes('keyquest-admin.onrender.com') || 
         window.location.hostname.includes('onrender.com'))) {
        
        console.warn('🚨 Using emergency fallback configuration for production deployment');
        console.warn('This indicates the build process did not inject environment variables properly');
        
        // Return placeholder config that will be replaced by build process
        const config = {
            url: '{{RENDER_SUPABASE_URL}}',
            anonKey: '{{RENDER_SUPABASE_ANON_KEY}}'
        };
        
        // If placeholders weren't replaced, it means build didn't run
        if (config.url.includes('{{') || config.anonKey.includes('{{')) {
            console.error('❌ Build process failed to replace environment variables');
            console.error('Please check Render build logs and environment variable configuration');
            throw new Error('Build process failed. Environment variables not injected.');
        }
    ...
    console.error('1. The build process did not run (missing config/env.js)');
    console.error('2. config/env.js is not loaded before this script');
    console.error('3. Environment variables are not set in Render dashboard');
    throw new Error('Environment configuration missing. Please contact administrator.');
};

const SUPABASE_CONFIG = getSupabaseConfig();

// Security Configuration - Now using build-time environment variables
const SECURITY_CONFIG = {
    sessionTimeout: (window.ENV_CONFIG && window.ENV_CONFIG.SESSION_TIMEOUT) || 3600000, // 1 hour
    maxLoginAttempts: (window.ENV_CONFIG && window.ENV_CONFIG.MAX_LOGIN_ATTEMPTS) || 5,
